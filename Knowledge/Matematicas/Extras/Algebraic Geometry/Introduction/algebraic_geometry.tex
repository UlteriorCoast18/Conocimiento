...
\newcommand\rad[1]{\ensuremath{\textup{rad}\left(#1\right)}}
...
\begin{document}
    ...
    \subfile{Chapter 1/chapter1.tex}
    ...
    \chapter{About Linear Functions}

    \begin{propo}
      Let $k$ be an algebraically closed field, then the set $\textup{Hom}(k^n,k)$ is isomorphic to $k^n$.
    \end{propo}

    \chapter{Fields}

    \begin{theor}[\textbf{Artin's Theorem}]
      Let $k$ be a field, then there exists a field $F$ such that $F$ is an algebraically closed extension of $k$.
    \end{theor}

    \begin{obs}
      Every algebraically closed field $F$ is infinite. Indeed, for every $n\in\bbm{N}$ the polynomial $x^n-1=0$ has a root in $F$ and, for $n\neq m$ we have that these roots are different one from another.
    \end{obs}

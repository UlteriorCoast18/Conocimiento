...
\begin{document}
    ...
    \begin{proof}
        ...
        \begin{itemize}
            \item Primero, veamos que significa la primera parte de la proposición. Podemos suponer (ya que $E_1$ es un disco cerrado), que $E_1$ es el cuadrado en $\mathbb{R}^2$:
            \begin{equation*}
                E_1=[-1,1]\times[-1,1]
            \end{equation*}
            Definimos sobre $E_1$ la relación $\mathcal{R}$ como sigue:
            \begin{equation*}
                \mathcal{R}=\bigcup_{ x\in[-1,1]\times]-1,1]\setminus\left\{(0,1)\right\} }\left\{x\right\}\cup\left\{[-1,1]\times\left\{-1\right\}\cup\left\{(0,-1)\right\} \right\}
            \end{equation*}
            (verifique que esta relación de equivalencia es efectivamente la que especifica la proposición). Afirmamos que $E_1/\mathcal{R}\cong\Delta$, donde $\Delta$ es el triángulo formado a partir de unir los puntos $(1,1)$, $(-1,1)$ y $(0,-1)$ en $\mathbb{R}^2$. En efecto, sea
            \begin{equation*}
                \cf{f}{E_1}{\Delta}
            \end{equation*}
            dada por:
            \begin{equation*}
                f(x,y) = \left(g(x,y),y \right),\quad\forall(x,y)\in E_1
            \end{equation*}
            donde
            \begin{equation*}
                \begin{split}
                    g(x,y) =\left\{ 
                        \begin{array}{lcr}
                            \frac{xy}{2} & \textup{ si } & x\geq0\\
                            -\frac{y+1}{2} & \textup{ si } & x<0 \\
                        \end{array}
                    \right.\quad\forall(x,y)\in E_1
                \end{split}
            \end{equation*}
            %TODO
            ...
            \begin{figure}
                \begin{center}
                    \includegraphics[scale=0.5]{images/fig_2.pdf}
                \end{center}
                \caption{Diagrama de $E_1$.}
            \end{figure}

            \begin{figure}
                \begin{center}
                    \includegraphics[scale=0.5]{images/fig_3.pdf}
                \end{center}
                \caption{Diagrama del conjunto $\Delta$.}
            \end{figure}

            Definimos $\cf{\overline{f}}{E_1/\mathcal{R}}{\Delta}$ dada por:
            \begin{equation*}
                \overline{f}([(x,y)])=f(x,y)
            \end{equation*}
            Donde $[(x,y)]$ es la clase de equivalencia de $(x,y)$. Se verifica rápidamente (\textit{ejercicio}) que $\overline{f}$ es biyección y más aún, que es homeomorfismo.

            \item Ya que todos los discos son homeomorfos, podemos elegir $E_1$ como queremos, en este caso, decidimos que $E_1$ es el trapezoide en $\mathbb{R}^2$ que une los puntos $ABDE$.
        \end{itemize}
    \end{proof}

    Estamos ahora listos para probar un lema fundamental. Sea $\cf{g}{I}{\partial\mathbb{D}^2}$ (donde $\partial\mathbb{D}^2$ denota la frontera de $\mathbb{D}^2$) la funcioń continua que da exactamente una vuelta alrededor del círculo, esto es
    \begin{equation*}
        g(0)=g(1)=d_0\in\partial\mathbb{D}^2
    \end{equation*}
    esto es, que $g$ mapea el intervalo $(0,1)$ de forma homeomorfa en $B\setminus\left\{d_0 \right\}$.

    \begin{lema}
        Sea $X$ un espacio topológico. Una funcioń continua $\cf{f}{\partial\mathbb{D}^2}{X}$ puede ser extendida a una función $\cf{F}{\mathbb{D}^2}{X}$ si y sólo si el bucle cerrado $\cf{f\circ g}{I}{X}$ es equivalente al bucle constante con punto base $f(d_0)$.
    \end{lema}

    \begin{proof}
        $\Rightarrow):$ Suponga que $\cf{f}{\partial\mathbb{D}^2}{X}$ puede ser extendida a una función $\cf{F}{\mathbb{D}^2}{X}$. Considere el cuadrado unitario:
        \begin{equation*}
            S=\left\{ (x,y)\in\mathbb{R}^2\Big|0\leq x\leq 1\textup{ y }0\leq y\leq 1 \right\}
        \end{equation*}
        Definimos una función continua $\cf{h}{S}{\partial\mathbb{D}^2}$ dada como sigue:
        \begin{equation*}
            \begin{split}
            ...
            \end{split}
        \end{equation*}
        Es claro que $h$ es continua. Por la proposición \ref{propB}, como $S\cong\mathbb{S}^1$, podemos extender $h$ a una función continua $\cf{H}{S}{\partial\mathbb{D}^2}$. La función continua $\cf{F\circ H}{I\times I}{X}$ satisface que:
        \begin{equation*}
            \begin{split}
    ...
    Aplicando el lema anterior, es conveniente usar el siguiente abuso de notación, diremos que la función $\cf{f}{\partial\mathbb{D}^2}{X}$ \textit{representa} la clase de equivalencia del bucle $f\circ g$.
    ...
    \begin{theor}
        ...
        \begin{minipage}{\textwidth}
            \begin{center}
                ...
                Figura 4. Conmutatividad de $\pi(X,x_0)$, $\pi(Y,\varphi_0(x_0))$ y $\pi(Y,\varphi_1(x_0))$.
            \end{center}
        \end{minipage}

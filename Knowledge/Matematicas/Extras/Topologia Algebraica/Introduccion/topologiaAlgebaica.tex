...
\begin{document}
    ...
    \section{Cómputo de algunos grupos fundamentales}
    ...
    \begin{mydef}
        ...
        \end{itemize}
        Cada abierto $U$ que satisfaga la condición anterior es llamado \textbf{vecindad elemental} y se dice que $U$ es \textbf{cubierto uniformemente}. La funcón $\cf{p}{\widetilde{X}}{X}$ es llamada habitualmente \textbf{proyección}.
    \end{mydef}
    ...
    \begin{obs}
        Para más ejemplos desarrollados más explícitamente, se recomienda checar: \textit{A basic course in Algebraic Topology} de Massey W. S.
    ...
    \begin{lema}
        Sea $(\widetilde{X},p)$ un espacio recubridor de $X$, $A$ un subespacio de $X$ que es arco-conexo y localmente arco-conexo. Sea $\widetilde{A}$ una componente arco-conexa de $p^{-1}(A)$, entonces $(\widetilde{A},p\Big|_{\widetilde{A}})$ es un espacio recubridor de $A$.
    \end{lema}
    ...
    \begin{proof}
        ...
        \begin{itemize}
            ...
            \end{equation*}
            es una cubierta abierta del compacto $I\subseteq\mathbb{R}$ (que puede ser visto como subespacio métrico de $\mathbb{R}$), luego existe 
        \end{itemize}
    \end{proof}
    ...
    Lo hecho en la parte anterior sobre espacios recubridores será utilizado para caracterizar en su totalidad el grupo fundamental

...
\begin{document}
    ...
    \begin{proof}
        ...
        \begin{equation*}
            \begin{split}
                \abs{\phi_{g_\nu}(h)}&\leq\int_{\mathbb{R}^n}\abs{hg_{\nu}}\\
                &=\int_{\mathbb{R}^n}\abs{h}\abs{f-\rho_\nu*f}\\
                &\leq\int_{\mathbb{R}^n}\abs{h(x)}dx\int_{\mathbb{R}^n}\abs{f(y)-f(x-y)}\rho_\nu(y)dy\\
                &\leq\int_{\mathbb{R}^n}\rho_\nu(y)dy\int_{\mathbb{R}^n}\abs{h(x)}\abs{f(y)-f(x-y)}dx\\
            \end{split}
        \end{equation*}
        donde se cambia el orden de integración. Se separa el producto por Hölder y se usa al continuidad uniforme de $y\mapsto f_{-y}$.

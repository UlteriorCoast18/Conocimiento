...
\begin{document}
    ...
    \begin{proof}
        Como $G$ es $\sigma$-compacto, para toda vecindad $U$ de $e_G$ existe un subconjunto a lo sumo numerable $F\subseteq G$ tal que
        \begin{equation*}
            G=U\cdot F
        \end{equation*}
        (recurde la definición de $\sigma$-compacto y compacto). Se sigue que se cumplen las hipótesis del teorema anterior con lo que, $w(G)=\chi(G)$.
    \end{proof}

    \begin{obs}
        Para un espacio topológico $X$ se define el \textbf{número de Lindelöf} como el cardinal más pequeño $l(X)$ tal que toda cubierta abierta admite una subcubierta de cardinalidad $l(X)$.
    \end{obs}

...
\begin{document}
    ...
    \begin{mydef}
        ...
        \begin{itemize}
            ...
            \begin{enumerate}
                \item \textit{Variables}: denotadsa por $v_1,...,v_n$.
                \item \textit{Conectivas}: a saber $\neg$ y $\Rightarrow$.
                \item \textit{Igualdad}: denotada por el símbolo $=$.
    ...
    \begin{mydef}
        ...
        \begin{enumerate}
            \item Cada $v_i$ y cada $c_i$ son términos (variables y constantes). Realmente estamos abreviando la cadena de longitud uno con único elemento a $v_i$ o $c_i$ (dependiendo del caso).
    ...
    \begin{exa}
        ...
        En $\mathcal{L}_A$, $S+1v_{ 57}$ es un término.
    ...
    \begin{equation*}
        (\forall v_5)(e_G=v_5(v_6e_G))
    \end{equation*}
    en esta fórmula, $v_5$ es llamada variable ligada, y $v_7$ es llamada libre.
    
    En la fórmula:
    \begin{equation*}
        (\forall x\forall y)(x+Sc(y)=Sc(x)+y)
    \end{equation*}
    no hay variables libres, todas son ligadas. Pero, en la fórmula
    \begin{equation*}
    ...
    \begin{mydef}
        Considere el conjunot de fórmulas. Entonces $\cf{Free}{\textup{Conjunto de Fórmulas}}{\mathcal{P}(\textup{Variables})}$ tal que

        \begin{equation*}
            Free(\textup{atómica})=\textup{Variables que aparecen en todos los términos}
        \end{equation*}

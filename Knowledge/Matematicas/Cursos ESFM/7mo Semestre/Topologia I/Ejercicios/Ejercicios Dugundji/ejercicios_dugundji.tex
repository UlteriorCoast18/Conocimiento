...
\begin{document}
    ...
    \begin{excer}
        Sean $(X_1,\tau_1)$ y $(X_1,\tau_2)$ espacios topológicos y $\cf{f}{(X_1,\tau_1)}{(X_2,\tau_2)}$ una función continua, suprayectiva y abierta. Entonces, $\tau_1=\tau_f$.
    \end{excer}
    ...
    \begin{theor}
        Sea $(X,\tau)$ un espacio topológico.
        \begin{enumerate}
            \item Sea $A\subseteq X$. Si existe una sucesión de puntos de $A$ que converge a $x$, entonces $x\in\Cls{A}$. El recíproco es cierto si $(X,\tau)$ es primero numerable.
            \item Sea $\cf{f}{X}{Y}$. Si $f$ es continua, entonces para cada sucesión convergente $\left\{x_n \right\}_{ n=1}^\infty\Rightarrow x$ en $X$, la sucesión $\left\{f(x_n) \right\}_{ n=1}^\infty \rightarrow f(x)$. El recíproco es cierto si $(X,\tau)$ es primero numerable.
        \end{enumerate}
    \end{theor}

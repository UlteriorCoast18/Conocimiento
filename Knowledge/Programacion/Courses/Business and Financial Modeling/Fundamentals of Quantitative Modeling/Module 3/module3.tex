...
\begin{document}
    ...
    Later on we will talk about Monte Carlo Simulations. Right now we will just pass to the next topic.

    \section{Markov Chain Models}
    
    \begin{mydef}[\textbf{Markov Chain}]
        A \textbf{Markov chain} is a dynamic probabilistic discrete model. This model describs a discrete time state space transition
    \end{mydef}

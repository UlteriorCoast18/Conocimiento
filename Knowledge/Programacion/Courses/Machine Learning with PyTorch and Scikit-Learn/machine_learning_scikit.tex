...
\begin{document}
    ...
    \subfile{Module 3/module3.tex}

    \subfile{Module 4/module4.tex}
...
'1.21.2'
    ...
    \chapter{A Note on Distributions}

    \section{Normal Distrubution}

    \section{Bernoulli's Distribution}

    \begin{mydef}[\textbf{Bernoulli distribution}]
        The \textbf{Bernoulli distribution}, named after Swiss mathematician Jacob Bernoulli, \textit{is the discrete probability distribution of a random variable which takes the value 1 with probability $p$ and the value $0$ with probability $q=1-p$}.
    \end{mydef}

    The Bernoulli distribution is a special case of the binomial distribution where a single trial is conducted (so $n$ would be 1 for such a binomial distribution). It is \textit{also a special case of the two-point distribution, for which the possible outcomes need not be 0 and 1}.
    ...
    The Bernoulli distrubution is simply $B(1,p)$, also written as $\textup{Bernoulli}(p)$.

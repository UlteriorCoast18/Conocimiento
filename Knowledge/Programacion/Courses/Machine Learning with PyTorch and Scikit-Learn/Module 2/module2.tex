...
Set labels and title for the Adaline plot, then display it.
    ...
    \begin{exa}[\textbf{Feature Scaling}]
    ...
    \begin{obs}[\textbf{Feature Scaling}]
        \textbf{Gradiant descent} is one of the many algorithms that benefit from \textbf{feature scaling}.
    \end{obs}

    \begin{mydef}[\textbf{Standarization}]
        \textbf{Standarization} \textit{is a normalization procedure that helps gradient descent learning converge more quickly}; however it \textit{does not make the original dataset normaly distributed}.
    \end{mydef}
    ...
    \begin{exa}[\textbf{Standarization Procedure}]
    ...
    \begin{obs}[Use of Standarization]
        One reason why standardization helps with gradient descent learning is that it \textbf{becomes easier to find a learning rate that works well for all weights} (and the bias). \textit{If the features are on vastly different scales, a learning rate that works well for updating one weight might be too large or too small to update another weight equally well. Using standardized features stabilizes the training so that the optimizer requires fewer steps to find a good or optimal solution} (\textbf{the global loss minimum}).  
    \end{obs}

    \begin{figure}[h]
        \begin{minipage}{\textwidth}
            \centering
            \includegraphics[scale=1]{images/global_loss_after_standarization.png} \\
            \caption{Global Loss Minimum with and without Zero Variance.}
            \label{figure:standarization_graphic}
        \end{minipage}
    \end{figure}

    Standardization can easily be achieved by using the built-in NumPy methods \lstinline|mean| and \lstinline|std|:
    \begin{lstlisting}[caption={Standarization Using Numpy},label=code:standarization_numpy]
X_std = np.copy(X)
X_std[:,0] = (X[:,0] - X[:,0].mean()) / X[:,0].std()
X_std[:,1] = (X[:,1] - X[:,1].mean()) / X[:,1].std()code
    \end{lstlisting}
    ...
    \begin{lstlisting}[caption={Adaline With Standarization Procedure},label=figure:adaline_standarization]
...
plt.show()
    ...
    \begin{figure}[h]
        \begin{minipage}{\textwidth}
            \centering
            \includegraphics[scale=1]{images/decision_after_standarization_adaline.png} \\
            \caption{Adeline after Standarization Procedure}
            \label{figure:adaline_after_standarization}
        \end{minipage}
    \end{figure}
    ...
    Previously, we minimized a loss function by taking a step in the opposite direction of the loss gradient that is calculated from the whole training dataset; this approach is sometimes referred to as \textbf{full batch gradient descent}.

    \begin{obs}[\textbf{Problem with Full Batch Gradient Descent}]
        For datasets with millions of data points, \textit{running full batch gradient descent can be computationally costly because the whole training dataset must be re-evaluated each time a step toward the global minimum is taken}.
    \end{obs}

    A popular alternative to batch gradient descent is \textbf{stochastic gradient descent} (\textbf{SGD}), sometimes \textit{called iterative or online gradient descent. Instead of updating the weights based on the sum of the accumulated errors over all training examples}, $x^{(i)}$:
    ...
    \begin{equation*}
        \Delta w_j=\eta(y^{(i)}-\sigma(z^{(i)}))x_j^{(i)},\quad\textup{and}\quad\Delta b=\eta(y^{(i)}-\sigma(z^{(i)}))
    \end{equation*}
    ...
    \begin{idea}[\textbf{Advantages of SGD}]
        Another advantage of SGD is that \textit{it supports online learning}. In online learning, \textit{the model is trained on the fly as new training data arrives}. This is \textbf{especially useful when accumulating large amounts of data}, for example, customer data in web applications. The \textit{system can immediately adapt to changes, and the training data can be discarded after updating the model if storage space is an issue}.
    \end{idea}
    ...
    Inside the \lstinline|fit| method, weights are updated after each training example. An additional \lstinline|partial_fit| method, which does not reinitialize the weights, supports online learning. To check whether the algorithm converged after training, the average loss of the training examples is calculated for each epoch. An option to shuffle the training data before each epoch avoids repetitive cycles when optimizing the loss function, and the \lstinline|random_state| parameter allows specification of a random seed for reproducibility:

    \begin{lstlisting}[caption={Adaline Implemented Using Stochastic Gradient Descent},label=adaline_stochastic]
"""ADAptive LInear NEuron classifier.
...
def predict(self, X):
    ...
    The \lstinline|_shuffle| method uses \lstinline|np.random.permutation| to generate a random sequence of unique indices, which are then applied to shuffle the feature matrix and class-label vector.

    \begin{obs}
        Display the plot with labeled axes using \lstinline|plt.show()|.
    \end{obs}

    \begin{figure}[h]
        \begin{minipage}{\textwidth}
            \centering
            \includegraphics[scale=1]{images/adaline_stochastic.png} \\
            \caption{Adaline Implementation using Stochastic Gradient DEscent}
            \label{figure:adaline_stochastic}
        \end{minipage}
    \end{figure}

    The \textit{average loss decreases quickly, and the final decision boundary after 15 epochs is similar to the batch gradient descent Adaline}.

    \begin{idea}[\textbf{Updating the Model in an Online Scenario}]
        To update the model in an online learning scenario with streaming data, call \lstinline|ada_sgd.partial_fit(X_std[0, :], y[0])| on individual training examples.
    \end{idea}

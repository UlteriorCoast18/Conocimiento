...
\begin{document}
    ...
    \begin{center}
      \textit{\lstinline|C++| is a compiled language. For a program to run, its source text has to be processed by a compiler, producing object ﬁles, which are combined by a linker yielding an executable program.}
    \end{center}
    ...
    \begin{obs}[\textbf{Portability of Source Code}]
      An executable program is created for a speciﬁc hardware/system combination; it is not portable, say, from a Mac to a Windows PC.

...
int main() {
  ...
  \begin{mydef}[\textbf{Inheritance}]
    \textbf{Inheritance} \textit{allows one class to reuse attributes and methods from another class}. It helps you write cleaner, more efficient code by avoiding duplication.
  \end{mydef}

  We group the \textit{inheritance concept} into two categories:

  \begin{itemize}
    \item \textbf{derived class (child)} - the \textit{class that inherits from another class}
    \item \textbf{base class (parent)} - the \textit{class being inherited from}
  \end{itemize}
  To inherit from a class, use the \lstinline|:| symbol.

  \begin{exa}[\textbf{Example of Inheritance}]
    In the code below, the \lstinline|Car| class (child) inherits the attributes and methods from the \lstinline|Vehicle| class (parent):
...
int main() {
  ...
  \begin{exa}[\textbf{Example of Multilevel Inheritance}]
    In the following example, \lstinline|MyGrandChild| is derived from class \lstinline|MyChild| (which is derived from \lstinline|MyClass|):
...
int main() {
  ...
  \subsection{C++ Multiple Inheritance}
...
int main() {
  ...
  \subsection{C++ Inheritance Access}

  You learned from the Access Specifiers chapter that there are three specifiers available in \lstinline|C++|.

  Until now, we have only used \lstinline|public| (members of a class are accessible from outside the class) and \lstinline|private| (members can only be accessed within the class).

  The third specifier, \lstinline|protected|, is similar to private, but it can also be accessed in the inherited class:
...
int main() {
  ...
  \textbf{Polymorphism} means \textit{many forms}, and it occurs when we have many classes that are related to each other by inheritance.

  Like we specified in the previous chapter; \textit{Inheritance lets us inherit attributes and methods from another class}. Polymorphism \textit{uses those methods to perform different tasks}. This allows us to \textit{perform a single action in different ways}.

  For example, imagine a base class \lstinline|Animal| with a method called \lstinline|makeSound()|. Derived classes of \lstinline|Animals| could be \lstinline|Pigs|, \lstinline|Cats|, \lstinline|Dogs|, \lstinline|Birds|, etc... Every animal can \textit{make a sound}, but each one sounds different:

  \begin{itemize}
    \item \lstinline|Pig|: wee wee
    \item \lstinline|Dog|: bow wow
    \item \lstinline|Bird|: tweet tweet
  \end{itemize}
...
class Dog : public Animal {
  ...
  Now we can create Pig and Dog objects and override the \lstinline|animalSound()| method:
...
int main() {
  ...
  \subsection{C++ Virtual Functions}

  \begin{mydef}[\textbf{Virtual Function}]
    A \textbf{virtual function} is a \textit{member function in the base class that can be overridden in derived classes}.
  \end{mydef}
  ...
  \begin{obs}[\textbf{Explanation of Virtual}]
    In \lstinline|C++|, when you write \lstinline|Shape* s = new Circle();|, you are storing a pointer to a \lstinline|Circle| object in a variable typed as \lstinline|Shape*|. 
    
    Without virtual functions, C++ decides which method to call based only on the static type of the pointer \lstinline|(Shape*)|, so calling \lstinline|s->draw()| will execute \lstinline|Shape::draw()| even though the object is actually a \lstinline|Circle|. This is because non-virtual methods use compile-time (static) binding. Virtual functions exist to change this behavior: when a method is declared virtual, \lstinline|C++| uses runtime (dynamic) binding and calls the method corresponding to the object’s real type, so \lstinline|s->draw()| correctly calls \lstinline|Circle::draw()|. In short, virtual functions allow derived objects to behave according to their actual type when accessed through base-class pointers or references.
  \end{obs}

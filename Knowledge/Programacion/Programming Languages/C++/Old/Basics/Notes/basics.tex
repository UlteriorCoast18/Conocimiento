...
cout << x + y + z;
    ...
    El operador \lstinline|<<| es el llamado \textbf{operador inserción (insertion operator)} y es usado para imprimir texto en la salida de comandos.

    \lstinline|C++| es una extensión de \lstinline|C|, por lo que tiene exactamente las mismas variables que \lstinline|C|.

    \newpage

    \section{User Input}

    \lstinline|cout| es para salida de texto en la consola de comandos, \lstinline|cin| es para la entrada de texto.

    \begin{mydef}[cir y $<<$]
        \lstinline|cin| es una variable predefinida que lee información del teclado con el \textbf{operador extracción (extraction operator)} \lstinline|>>|
    \end{mydef}

    Con este ejemplo imprimimos el valor de \lstinline|x|, antes guardamos la entrada de texto en esta variable.
